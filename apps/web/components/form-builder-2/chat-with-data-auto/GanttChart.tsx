...
export default function GanttChart({ data }: { data: TaskData[] }) {
  // Initialize currentDate to the start date of the first task, or today if no tasks
  const [viewType, setViewType] = useState("month");
  const [currentDate, setCurrentDate] = useState(() => {
    if (
      data?.length > 0 &&
      data[0]?.plan_start_date &&
      typeof data[0].plan_start_date === "string"
    ) {
      try {
        const parsedDate = parseISO(data[0].plan_start_date);
  ...
  const validateAndParseDate = (dateString: any): Date | null => {
        ...
        <div className="overflow-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map((item) => {
                const startDate = validateAndParseDate(item.plan_start_date);
                const endDate = validateAndParseDate(item.plan_end_date);
