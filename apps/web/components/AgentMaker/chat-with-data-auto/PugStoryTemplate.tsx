...
const PugStoryTemplate = ({ data, componentName }: PugTemplateProps) => {
  ...
  const generateNarrative = (template: string) => {
    if (!template.trim()) {
      toast({ description: "Please enter a template", variant: "destructive" });
      return;
    }
  ...
  const generateStory = async () => {
    try {
      ...
      if (!pugStory || !data) {
        toast({
          variant: "destructive",
          description: "Please generate a template and data first",
        });
        return;
      }
      ...
      if (result) {
        ...
        toast({
          variant: "destructive",
          description: "An error occurred while generating AI story",
        });
      }
    } catch (err) {
      toast({
