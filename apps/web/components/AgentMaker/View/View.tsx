...
import { NEXT_PUBLIC_API_KEY, SAVE_FORM_DATA, ADD_FORM_DATA } from "@/constants/envConfig";
...
const View = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch form setup
        const setupResponse = await fetch(`${SAVE_FORM_DATA}?form_id=eq.${currentId}`, {
          headers: {
        ...
        if (setupData[0] && setupData[0].custom_one) {
          const customColumns = JSON.parse(setupData[0].custom_one).map((columnName: string) => ({
        ...
        // Fetch form data
        const dataResponse = await fetch(`${ADD_FORM_DATA}?form_id=eq.${currentId}`, {
          headers: {
            Authorization: `Bearer ${NEXT_PUBLIC_API_KEY}`,
          },
        });
